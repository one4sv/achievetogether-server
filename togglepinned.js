...
export default function(app, supabase) {
    app.post("/togglepinned", authenticateUser(supabase), async (req, res) => {
        ...
        try {
            ...
            const { data: oldPinned, error: pinned_error } = await supabase
                .from("chat_members")
                .select("pinned")
                .eq("user_id", id1)
                .eq("chat_id", chatId)
            ...
            if (pinned_error || !oldPinned) {
                return res.status(404).json({ success: false, error: "Ошибка получения pinned" });
            }

            const { error:update_error } = await supabase
                .from("chat_members")
                .update({ pinned: !oldPinned.pinned })
                .eq("user_id", id1)
                .eq("chat_id", chatId)
